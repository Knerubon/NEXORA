    ...
    return StructureSnapshot(
        ...
        pivots=tuple(
            ConfirmedPivot(
                kind=kind,
                price=Decimal(price),
                occurrence_time=NOW + timedelta(minutes=i * 2),
                confirmation_time=NOW + timedelta(minutes=i * 2 + 1),
                source_transition_id=f"pivot-{i}",
                config_version="p3-fixed-v1",
            )
            for i, (kind, price, _status) in enumerate(pivots, 1)
        ),
        levels=tuple(
            CandidateLevel(
                side=side,
                price=Decimal(price),
                status=status,
                source_pivot_id=f"level-{i}",
                updated_at=NOW + timedelta(minutes=i),
            )
            for i, (side, price, status) in enumerate(levels, 1)
        ),
    )


structure = st(
    (("low", "100.0", "confirmed"), ("high", "104.0", "confirmed"), ("low", "100.1", "confirmed")),
    (("support", "99.8", "confirmed"), ("resistance", "104.2", "confirmed")),
)
assessment = SignalEngine(cfg())._assess_components(
    structure=structure, regime=reg("range"), matrix=mat("mixed", direction="X", price="103.9")
)
